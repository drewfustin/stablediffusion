...
def inpaint(
    sampler, image, mask, prompt, seed, scale, ddim_steps, num_samples=1, w=512, h=512, eta=1.0
):
    device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    model = sampler.model
...
def run():
    ...
    if image:
        ...
        stroke_width = st.number_input("Brush Size", value=64, min_value=1, max_value=100)
        ...
        st.write("Canvas")
        st.caption(
            "Draw a mask to inpaint, then click the 'Send to Streamlit' button "
            "(bottom left, with an arrow on it)."
        )
